<template>
  <form @submit.prevent.stop="onSubmit">
    <div>
     ...
     <div>
      <label :for="passworId">Password:</label>
      <input :id="passworId" name="password" @blur="validatePassword" v-model="password" type="text" />
...
const isPassword = (value, msg = 'Formato de password incorrecto') => {
    const isValidPassword = /^(?=\w*\d)(?=\w*[A-Z])(?=\w*[a-z])\S{8,16}$/.test(value);
...
export default {
    ...
    methods: {
        ...
        validatePassword(){
            this.passwordError = isPassword(this.password, 'formato del password incorrecto');
        }
    }
}
</script>
